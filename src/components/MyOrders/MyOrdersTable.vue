    ...
    <vs-table>
      ...
      <template #tbody>
        <vs-tr v-for="(order, i) in getPage(filteredOrders, page, pageSize)" :key="i" :data="order">
          ...
          <vs-td :style="{ backgroundColor: getStatusColor(order.status) }">
            {{ order.status }}
          </vs-td>
        </vs-tr>
      </template>
...
<script>
  ...
  export default {
    ...
    setup() {
      ...
      // Define colors for different status backgrounds
      const statusColors = {
        Pending: '#ffcc00', // Yellow for pending status
        Completed: '#4caf50', // Green for completed status
        Scheduled: '#3498db', // Blue for scheduled status
